...
Write-Host "== chunked ingest resume/idempotency + restart durability =="
@'
import base64
import json
import sqlite3
...
docker compose restart mynahd | Out-Null
...
req = urllib.request.Request(
    "http://mynahd:8001/ingest/audio_chunk",
    data=json.dumps(chunk0).encode("utf-8"),
    headers={"Content-Type": "application/json"},
    method="POST",
)
with urllib.request.urlopen(req, timeout=10) as resp:
...
Write-Host "== ingest fixture audio and transcribe =="
@'
import base64
import json
import urllib.request
...
Write-Host "== report generation and visibility =="
@'
import json
import urllib.request
from datetime import datetime, timezone
...
Write-Host "== sql tool accept/reject/audit =="
@'
import json
import urllib.error
import urllib.request
...
Write-Host "== stale memory exclusion and reverification =="
@'
import json
import sqlite3
import time
